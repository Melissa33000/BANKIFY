...
if(!empty($_POST)){
    ...
    if(empty($_POST['civilite']) && empty($_POST['nom']) && empty($_POST['prenom']) && empty($_POST['dateN']) && empty($_POST['email']) && empty($_POST['mdp']) && empty($_POST['confirmmdp'])){
        $erreur = true;
    }
    ...
    if(isset($_POST['email'])) {
        ...
        $resultVerifMail = $dbConn->query($queryVerifMail);
        if ($row = $resultVerifMail->fetchObject()) {
            ...
            if ($nbrMailBDD > 0) {
                ...
                // ETAPES pour le remplissage de la table CPVILLE dans la BDD
                if($cp != null and $ville != null ) {

                    $SQLQuery = 'INSERT INTO cpville(ville, cp) VALUES (:ville, :cp)';
                    $SQLStatement = $dbConn->prepare($SQLQuery);
                    $SQLStatement->bindValue(':ville', $ville);
                    $SQLStatement->bindValue(':cp', $cp);

                    // Exécution de la requête :
                    if (!$SQLStatement->execute()) {
                        print("Erreur d'exécution de la requête d'insertion de la Ville !<br />");
                        var_dump($SQLStatement->errorInfo());
                        die();
                    }
                }

                // ETAPES pour le remplissage de la table PAYS dans la BDD
                if($pays != null ){

                    $SQLQuery = 'INSERT INTO pays(nom) VALUES (:pays)';
                    $SQLStatement = $dbConn->prepare($SQLQuery);
                    $SQLStatement->bindValue(':pays', $pays);

                    // Exécution de la requête :
                    if (!$SQLStatement->execute()) {
                        print("Erreur d'exécution de la requête d'insertion du Pays !<br />");
                        var_dump($SQLStatement->errorInfo());
                        die();
                    }
                }

                // ETAPES pour le remplissage de la table AVOIR dans la BDD
                $SQLQuery = 'INSERT INTO avoir(id_cpville, id_pays) VALUES ((SELECT id FROM cpville WHERE id = last_insert_id()), (SELECT id FROM pays WHERE id = last_insert_id()))';
                ...
                if($adresse != null and $adresse2 != null ){
                    ...
                    $SQLQuery = 'INSERT INTO adresse(ligne_adresse, ligne_adresse2, id_cpville) VALUES (:adresse_1, :adresse_2, (SELECT id_cpville FROM avoir WHERE id_cpville =last_insert_id()))';
                    $SQLStatement = $dbConn->prepare($SQLQuery);
...
<div class="formDesign formAjust">
    ...
    <form id="inscription" action="" method="post" onsubmit="return verif_formulaire_profil()">
        ...
        <div class="champs_form">
            ...
            <div class="champ_form">
                <input type="number" name="cp" maxlength="5" id="cp"/><br/><br/>
            </div>
            ...
            <div class="champ_form">
                <input type="text" name="ville" id="ville" maxlength="50" onkeyup="return verif_ville()"/><br/>
                <div class="message_erreur" id="error_ville_contenu"></div>
            </div>

            <div class="label_form">
                <label class="labelprofil"  for="pays">Pays :</label>
            </div>
            <div class="champ_form">
                <input type="text" name="pays" id="pays" maxlength="50" onkeyup="return verif_pays()"/><br/>
                <div class="message_erreur" id="error_pays_contenu"></div>
            </div>
